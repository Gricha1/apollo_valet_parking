...
function main() {
    ...
    ${DOCKER_RUN_CMD} -itd \
        ...
        -e HOST_DREAMVIEW_PORT="${HOST_DREAMVIEW_PORT}" \
        -e HOST_BRIDGE_PORT="${HOST_BRIDGE_PORT}" \
        -e HOST_IP="${HOST_IP}" \
        ${MAP_VOLUMES_CONF} \
        ${OTHER_VOLUMES_CONF} \
        ${local_volumes} \
        ...
        -p "${HOST_DREAMVIEW_PORT}:${DREAMVIEW_PORT}" \
        -p "${HOST_BRIDGE_PORT}:${BRIDGE_PORT}" \
        --hostname "${DEV_INSIDE}" \
        --shm-size "${SHM_SIZE}" \
        "${DEV_IMAGE}" \
        /bin/bash
